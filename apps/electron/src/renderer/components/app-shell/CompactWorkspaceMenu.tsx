...
import { resolveCompactWorkspaceSelection, type CompactWorkspaceSelection } from './compact-workspace-navigation'
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <TopBarButton
          aria-label={`${t('serviceRail.title')} · ${t('panelWorkspace.openPanels')}`}
      ...
      >
        {tabs.length > 0 && (
          <>
            <div role="group" aria-label={t('panelWorkspace.openPanels')}>
              <div className="px-2 py-1.5 text-xs font-medium text-muted-foreground" aria-hidden>{t('panelWorkspace.openPanels')}</div>
              {tabs.map((tab, index) => {
                ...
                const title = tab.kind === null && service ? t(service.railLabelKey ?? service.labelKey) : tab.title
                return (
                  <StyledDropdownMenuItem
                    key={tab.panelId}
        ...
        <div role="group" aria-label={t('serviceRail.title')}>
          <div className="px-2 py-1.5 text-xs font-medium text-muted-foreground" aria-hidden>{t('serviceRail.title')}</div>
