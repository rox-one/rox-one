...
describe('compact workspace navigation', () => {
  it('keeps every registered service reachable without a desktop rail or sidebar', () => {
    for (const destination of APP_NAV_DESTINATIONS) {
      ...
      if ('action' in destination && destination.action === 'open-browser') {
        expect(action).toEqual({ kind: 'open-browser' })
  ...
  it('selects any open panel and preserves its identity, route, filter and sizing', () => {
    ...
    for (const panel of panels) {
      const action = resolveCompactWorkspaceSelection(panels, store.get(focusedPanelIdAtom), {
        kind: 'panel',
        panelId: panel.id,
      })
  ...
  it('reuses an open service before calling its normal opener', () => {
    const store = createStore()
    store.set(pushPanelAtom, { route: routes.view.notes('draft') })
    store.set(pushPanelAtom, { route: routes.view.allSessions('chat') })
    const panels = store.get(panelStackAtom)
    ...
    expect(
      resolveCompactWorkspaceSelection(panels, panels[1].id, { kind: 'service', serviceId: 'settings' }),
    ).toEqual({ kind: 'navigate', route: routes.view.settings() })
  })
})
