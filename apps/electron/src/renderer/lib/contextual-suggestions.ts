...
const STOP_WORDS: Record<string, true> = {
  about: true, after: true, also: true, before: true, create: true, from: true, help: true, into: true, please: true, project: true, should: true, skill: true, that: true, their: true, them: true, these: true, this: true, using: true, want: true, when: true, with: true, your: true,
  быть: true, будет: true, всего: true, давай: true, если: true, используй: true, который: true, можно: true, нужно: true, навык: true, пожалуйста: true, помоги: true, после: true, проект: true, сделай: true, этого: true, этот: true, чтобы: true,
}
...
function meaningfulWords(text: string): Set<string> {
  return new Set(normalizeText(text).split(' ').filter(word => word.length >= 4 && !STOP_WORDS[word]))
}
