...
describe('CF-6.2 ConnectionsPage', () => {
  ...
  it('shows tenant workspace id on policy connection rows without secret fields', () => {
  ...
  it('shows inspect health on policy connection rows without secret fields', () => {
    expect(page).toContain('connections-policy-health')
    expect(page).toContain('inspectById[row.id].health')
    expect(page.toLowerCase()).not.toContain('infisical')
    expect(page).not.toMatch(/\bpayload\b|\bsecret\b|\brefreshToken\b/)
  })

  it('shows inspect expiry on policy connection rows without secret fields', () => {
  ...
  it('shows inspect provenance on policy connection rows without secret fields', () => {
  ...
  it('shows inspect kind on policy connection rows without secret fields', () => {
  ...
  it('shows inspect fingerprint on policy connection rows without secret fields', () => {
  ...
  it('shows inspect version on policy connection rows without secret fields', () => {
  ...
  it('hides empty revalidation on policy connection rows without secret fields', () => {
  ...
  it('uses dedicated reconnect testids on credential and policy rows without stealing focus', () => {
    expect(page.split('{renderReconnectControls(row,').length - 1).toBe(3)
    expect(page).toContain("'connections-row-reconnect'")
  ...
  it('uses dedicated rotate confirm-target testids on credential and policy rows without secret fields', () => {
    expect(page.split('{renderRotateControls(row,').length - 1).toBe(3)
    ...
    expect(page).not.toContain('data-testid="connections-rotate-confirm-target"')
    expect(page.toLowerCase()).not.toContain('infisical')
    expect(page).not.toMatch(/\bpayload\b|\bsecret\b|\brefreshToken\b/)
  })
  ...
  it('offers test and repair on credential rows without stealing focus', () => {
    ...
    expect(page.split('runTest(row.id)').length - 1).toBe(1)
    expect(page.split('runRepair(row.id)').length - 1).toBe(1)
    expect(page).toContain("'connections-row-test'")
    expect(page).toContain("'connections-credential-test'")
    expect(page).toContain("'connections-policy-test'")
    expect(page).toContain("'connections-row-repair'")
    expect(page).toContain("'connections-credential-repair'")
    expect(page).toContain("'connections-policy-repair'")
  ...
  it('uses dedicated test and repair testids on listed rows without secret fields', () => {
    expect(page.split('{renderTestRepairControls(row,').length - 1).toBe(3)
    expect(page).toContain("'connections-row-test'")
    expect(page).toContain("'connections-credential-test'")
    expect(page).toContain("'connections-policy-test'")
  ...
  it('shares test and repair controls through one helper without secret fields', () => {
    expect(page).toContain('const renderTestRepairControls')
    expect(page.split('{renderTestRepairControls(row,').length - 1).toBe(3)
    expect(page.split('runTest(row.id)').length - 1).toBe(1)
    expect(page.split('runRepair(row.id)').length - 1).toBe(1)
    expect(page).toContain("'connections-row-test'")
  ...
  it('shows tenant workspace id on credential rows without secret fields', () => {
  ...
  it('labels invalidated leases and revalidation on listed rows through i18n without secret fields', () => {
    expect(page.split("t('inspector.field.leases')").length - 1).toBe(1)
    expect(page.split("t('inspector.field.revalidated')").length - 1).toBe(1)
    expect(page.split('{renderLeaseRevalidated(row,').length - 1).toBe(3)
  ...
  it('shares lease and revalidation display through one helper without secret fields', () => {
    expect(page).toContain('const renderLeaseRevalidated')
    expect(page.split('{renderLeaseRevalidated(row,').length - 1).toBe(3)
    expect(page.split("t('inspector.field.leases')").length - 1).toBe(1)
    expect(page.split("t('inspector.field.revalidated')").length - 1).toBe(1)
    expect(page).toContain("'connections-row-leases'")
  ...
  it('labels test outcome on listed rows through i18n without secret fields', () => {
    expect(page.split("t('inspector.field.testLogin')").length - 1).toBe(1)
    expect(page).toContain('connections-test-status')
    expect(page).toContain('connections-credential-test-status')
    expect(page).toContain('connections-policy-test-status')
    expect(page).toContain("const TABS = ['services', 'credentials', 'imports', 'policies', 'audit']")
  ...
  it('shares test outcome display through one helper without secret fields', () => {
    expect(page).toContain('const renderTestStatus')
    expect(page.split('{renderTestStatus(status,').length - 1).toBe(3)
    expect(page.split("t('inspector.field.testLogin')").length - 1).toBe(1)
    expect(page).toContain("'connections-test-status'")
    expect(page).toContain("'connections-credential-test-status'")
    expect(page).toContain("'connections-policy-test-status'")
