...
describe('CF-6 Connections UI helpers', () => {
  ...
  it('never echoes folder-picker errors into toast descriptions', () => {
    expect(folderPickerErrorDescription(new Error('/secret/path token=gho_leak'))).toBeUndefined()
    expect(folderPickerErrorDescription({ message: 'EACCES /Users/me/.ssh/id_rsa' })).toBeUndefined()
    expect(folderPickerErrorDescription('dialog cancelled')).toBeUndefined()
  })
