...
describe('ROX2-021 native sessions surface', () => {
  ...
  test('native session bind and empty list are live; fixture/conation are not', () => {
    ...
    expect(isClaimableLive(nativeSessionListResult([]))).toBe(false)
    expect(isClaimableLive(nativeSessionListResult([entity]))).toBe(false)
    ...
    expect(isClaimableLive(sessionSurfaceResult('native'))).toBe(false)
  })
})
