...
describe('ROX2 native rail and browser surfaces', () => {
  ...
  test('native binds and empty lists are live; fixture/conation are not', () => {
    for (const id of NATIVE_RAIL_SURFACE_IDS) {
      ...
      expect(isClaimableLive(nativeSurfaceListResult(id, []))).toBe(true)
      expect(isClaimableLive(nativeSurfaceResult(id, 'native'))).toBe(true)
    ...
    expect(isClaimableLive(nativeSurfaceResult('browser', 'native'))).toBe(true)
