...
describe('issues 07–09 wiring', () => {
  ...
  it('hosts table, canvas, outline and graph views without replacing Map', () => {
    ...
    expect(host).toContain('notes-canvas-zoom-in')
    expect(host).toContain('notes-canvas-zoom-out')
  ...
  it('wires column keyboard, grouped @ palette and comment rail', () => {
    ...
    expect(page).toContain("t('notes.vault.noMatches')")
    expect(page).toContain("t('notes.vault.noteCount'")
    expect(page).toContain("t('notes.vault.assetCount'")
    expect(page).not.toContain('No matching notes')
    expect(page).not.toContain('No notes yet')
  })
  ...
  it('exposes playground stories for inspector, wiki, palette, comments and formula table', () => {
    ...
    expect(notes).toContain("id: 'notes-table-empty'")
    expect(notes).toContain("id: 'notes-outline-empty'")
    expect(notes).toContain('NotesEmptyPlayground')
  ...
  it('wires note convert to real personal task and session (not toast/summarize-only)', () => {
    const notesPage = read('pages/NotesPage.tsx')
    expect(notesPage).toContain('PersonalTaskStore')
    expect(notesPage).toContain('persistPersonalTaskStore')
    expect(notesPage).toContain('routes.view.tasks(created.id)')
    expect(notesPage).toContain('provenance.noteId')
    expect(notesPage).toContain('routes.view.allSessions(session.id)')
    expect(notesPage).not.toContain("if (converted.kind === 'session-draft') void handleAskAgent('summarize')")
    expect(notesPage).not.toContain("else toast.success(t('notes.views.convertTaskDone'))")
  })
})
