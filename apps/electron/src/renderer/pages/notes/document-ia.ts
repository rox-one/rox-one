...
export function locateColumnBlocks(markdown: string): LocatedColumnLayout[] {
  ...
  const re = /^:::columns\s+(\d+)\s*\n[\s\S]*?^:::\s*$/gm
  let match: RegExpExecArray | null
  while ((match = re.exec(markdown)) !== null) {
    const parsed = extractColumns(match[0]!)[0]
    if (!parsed) continue
    layouts.push({ ...parsed, start: match.index, end: match.index + match[0].length })
  }
  return layouts
}
...
export function notePaletteKeyAction(
  ...
  if (event.key === 'ArrowDown') return { type: 'move', index: Math.min(index + 1, count - 1) }
...
export function noteCommentKeyboardAction(event: {
  ...
  const mod = event.metaKey || event.ctrlKey
  if (mod && event.shiftKey && (event.key === 'm' || event.key === 'M')) return 'open'
  return null
}
