...
describe('CF-6.4 InspectorHost connections', () => {
  ...
  it('hides empty inspect fields in the inspector without secret fields', () => {
    ...
    expect(host.split("value !== '—'").length - 1).toBe(0)
    expect(host).not.toContain('function visibleInspectValue')
    expect(host).toContain('visibleInspectValue,')
    expect(host).toContain('connections-inspector-health')
    expect(host.toLowerCase()).not.toContain('infisical')
    expect(host).not.toMatch(/\bpayload\b|\bsecret\b|\brefreshToken\b/)
  ...
  it('exposes inspect expiry, provenance, fingerprint, kind, and version testids without secret fields', () => {
    expect(host).toContain('connections-inspector-expiry')
    expect(host).toContain('connections-inspector-provenance')
    expect(host).toContain('connections-inspector-fingerprint')
    expect(host).toContain('connections-inspector-kind')
    expect(host).toContain('connections-inspector-version')
    expect(host).toContain('inspector.field.expiry')
    expect(host).toContain('inspector.field.provenance')
    expect(host).toContain('inspector.field.fingerprint')
    expect(host).toContain('inspector.field.credentialKind')
    expect(host).toContain('inspector.field.versionId')
  ...
  it('offers an in-place reconnect CTA for stale inspect without stealing focus', () => {
    ...
    expect(host).toContain('connections-inspector-reconnect-confirm-target')
    expect(host).not.toContain('data-testid="connections-reconnect-confirm-target"')
    expect(host).toContain('formatConfirmLeases')
    expect(host).not.toContain('autoFocus')
    expect(host).not.toContain('bringToFront')
    expect(host).not.toContain('bring_to_front')
  ...
  it('hides empty inspector grant purpose, actions, and resources without secret fields', () => {
    ...
    expect(host).not.toContain("row.actions.join(', ') || '—'")
    expect(host).not.toContain("row.resources.join(', ') || '—'")
    expect(host).toContain('connections-inspector-purpose')
    expect(host).toContain('connections-inspector-actions')
    expect(host).toContain('connections-inspector-resources')
    expect(host.toLowerCase()).not.toContain('infisical')
  ...
  it('hides empty inspector grant actions and resources without an em-dash placeholder without secret fields', () => {
    expect(host).not.toContain("row.actions.join(', ') || '—'")
    expect(host).not.toContain("row.resources.join(', ') || '—'")
    expect(host).toContain("row.actions.join(', ')")
    expect(host).toContain("row.resources.join(', ')")
    expect(host).toContain('visibleInspectValue(row.actions)')
    expect(host).toContain('visibleInspectValue(row.resources)')
    expect(host.toLowerCase()).not.toContain('infisical')
    expect(host).not.toMatch(/\bpayload\b|\bsecret\b|\brefreshToken\b/)
  ...
  it('exposes inspector provider, storage, credential ref, and scopes testids without secret fields', () => {
    ...
    expect(host).toContain('connections-inspector-scopes')
    expect(host).toContain('inspector.field.provider')
    expect(host).toContain('inspector.field.storageMode')
    expect(host).toContain('inspector.field.credentialRef')
    expect(host).toContain('inspector.field.scopes')
    expect(host).toContain('fields.provider')
    expect(host).toContain('fields.storageMode')
    expect(host).toContain('fields.credentialRef')
    expect(host).toContain('fields.scopes')
    expect(host).not.toMatch(/\/token\//)
    expect(host.toLowerCase()).not.toContain('infisical')
    expect(host).not.toMatch(/\bpayload\b|\bsecret\b|\brefreshToken\b/)
  })

  it('hides empty inspector scopes without secret fields', () => {
    expect(host).toContain('visibleInspectValue(fields.scopes)')
    expect(host).toContain('connections-inspector-scopes')
    expect(host).toContain('inspector.field.scopes')
    expect(host.toLowerCase()).not.toContain('infisical')
    expect(host).not.toMatch(/\bpayload\b|\bsecret\b|\brefreshToken\b/)
