...
- Source-folder FS watcher. **Landed behind `CRAFT_FEATURE_NATIVE_INDEX_WATCH=1`:** TS watches local source roots (cap 32), debounce 250ms, skips `node_modules`/`.git`/binaries, reindexes via the facade (native incremental when primary). Push `sources:indexChanged`. Not a knowledge watcher and not inside `craft-index`.
