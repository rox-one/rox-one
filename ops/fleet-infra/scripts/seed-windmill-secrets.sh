...
environment="prod"
node_name="${FLEET_NODE_NAME:-automation-01}"
secret_path="/nodes/${node_name}/windmill"
...
payload_file="$(mktemp "$runtime_dir/rox-windmill-secret.XXXXXX")"
chmod 0600 "$payload_file"

cleanup() {
  unset api_token secret_value
  if command -v shred >/dev/null 2>&1; then
    shred -u "$payload_file" 2>/dev/null || true
  else
    rm -f "$payload_file"
  fi
}
...
secret_exists() {
  local name="$1" path_encoded
  path_encoded="$(jq -rn --arg v "$secret_path" '$v|@uri')"
  curl --config <(curl_config) --output /dev/null \
...
create_secret() {
  ...
  jq -nc --arg workspace "$project_id" --arg env "$environment" --arg path "$secret_path" --arg value "$secret_value" \
    '{workspaceId:$workspace,environment:$env,secretPath:$path,secretValue:$value,type:"shared",secretComment:"Managed by fleet-infra; never copy to Git"}' >"$payload_file"
...
ensure_folder "/nodes/${node_name}" windmill
create_secret WINDMILL_DB_PASSWORD 36
create_secret WINDMILL_SUPERADMIN_SECRET 36
create_secret WINDMILL_ADMIN_PASSWORD 24
