...
describe('calendar-calls (#382) fail-closed', () => {
  ...
  it('canceled write stays blocked as calendar-canceled-not-live, not verified/live/L4', () => {
    const canceled = applyCalendarWrite(occ({ canceled: true }))
    expect(canceled.status).toBe('blocked')
    expect(canceled.reason).toBe('calendar-canceled-not-live')
    expect(canceled.status).not.toBe('verified')
    expect(canceled.live).toBe(false)
    expect(canceled.live).not.toBe(true)
    expect(canceled.evidenceLevel).toBe('U1')
    expect(canceled.evidenceLevel).not.toBe('L4')
    expect(isLiveVerified(canceled)).toBe(false)
  })

  it('call bind stays blocked as call-bind-not-live, not verified/live/L4', () => {
    const seen = new Map<string, string>()
    const bound = bindCall('mtg-1', 'call-1', seen)
    expect(bound.status).toBe('blocked')
    expect(bound.reason).toBe('call-bind-not-live')
    expect(bound.status).not.toBe('verified')
    expect(bound.live).toBe(false)
    expect(bound.live).not.toBe(true)
    expect(bound.evidenceLevel).toBe('U1')
    expect(bound.evidenceLevel).not.toBe('L4')
    expect(isLiveVerified(bound)).toBe(false)
