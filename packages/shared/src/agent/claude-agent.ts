...
export class ClaudeAgent extends BaseAgent {
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      // for OAuth the [1m] model suffix is the way (both auth paths). See:
      // anthropics/claude-agent-sdk-typescript#238
      // Gated by enable1MContext in global config (~/.craft-agent/config.json).
      // Opt-in only: undefined/false must not append [1m] (matches getEnable1MContext /
      // is1MContextEnabled). The interceptor also reads this to strip the SDK beta header.
