...
const KEY = 'cloudRuns.error'
const RU_VALUE = 'Ошибка облачного запуска'
const EN_VALUE = 'Cloud run error'
...
describe('P35-221 leftover Russian ран wrapping on cloudRuns.error', () => {
  it('keeps English catalog copy unchanged', () => {
    expect(en[KEY]).toBe(EN_VALUE)
    expect(en[KEY]).toContain('Cloud run error')
    expect(en[KEY]).not.toContain('запуска')
  })

  it('wraps leftover ран as sibling запуск on cloudRuns.error only', () => {
    ...
    expect(ru[KEY]).not.toBe('Ошибка облачного рана')
  })
  ...
  it('resolves English through setupI18n after changeLanguage(en)', async () => {
    ...
    expect(i18n.t(KEY)).toContain('Cloud run error')
