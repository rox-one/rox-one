...
const KEY = 'collection.bulk.unflag'
const RU_WRAPPED = 'Снять пометку'
const EN_VALUE = 'Unflag'

describe('P35-351 leftover Russian флаг wrapping on collection.bulk.unflag', () => {
  it('wraps leftover флаг as sibling пометка, then English stays English', async () => {
    await setupI18n().changeLanguage('ru')
    const ru = i18n.t(KEY)
    expect(ru).toBe(RU_WRAPPED)
    expect(ru.toLowerCase()).toContain('пометку')
