...
describe('terminology linter', () => {
  it('does not flag allowlisted compatibility locale keys', () => {
    ...
    expect(localeValueViolations('cloudRuns.omp', 'OMP runner')).toEqual([])
    expect(localeValueViolations('cloudRuns.ompHint', 'Run the OMP agent loop inside the Daytona sandbox')).toEqual([])
