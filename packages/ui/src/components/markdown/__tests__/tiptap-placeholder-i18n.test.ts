...
const ru = JSON.parse(
  readFileSync(join(import.meta.dir, '../../../../../shared/src/i18n/locales/ru.json'), 'utf8'),
) as Record<string, string>
...
describe('TiptapMarkdownEditor placeholder default is i18n', () => {
  ...
  it('keeps Russian copy distinct and does not reuse the notes placeholder', () => {
    expect(source).not.toContain('notes.editor.placeholder')
    expect(en['notes.editor.placeholder']).toBe('Write note…')
    expect(ru['editor.placeholder']).toBe('Пишите что-нибудь...')
    expect(ru['editor.placeholder']).not.toBe(en['editor.placeholder'])
  })
})
