...
- **R2 (browser): PASS — P0 verified fixed from the user perspective.** Login page Rox-branded and ru-default; "ping test" fails bounded in ~2 s with the actionable `OMP_NO_MODELS` message; session returns to idle (input usable); no `[StaleSecurity]` warnings; reload preserves state. Known minor: `/manifest*` 401s (pre-existing, cosmetic).
