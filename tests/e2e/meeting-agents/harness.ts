...
function resolveEntrypoint(): { path: string; kind: MeetingAppHandles['entrypoint'] } {
  ...
  // Gate G1: never default to fixture as if it were product E3.
  if (!wantFixture) {
    throw new BootMeetingAppError(
      'fail-closed',
      'E3 blocked — no product Electron→RPC→storage path; set ROX_MEETING_E2E_FIXTURE=1 for U1 fixture only (never E3) or ROX_MEETING_USE_PACKAGED_APP=1 for product',
    )
  }
